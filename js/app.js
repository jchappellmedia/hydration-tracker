    ...
    render(r){
      ...
      const upsell = reviewLocked ? `
        <div class="upsell">
          ...
          <div>
            <b>Your next try will be better.</b>
            <p>Nobody aces this on the first go. Practice again and again until the questions stop feeling new. Pro unlocks unlimited full tests, every question in the bank, and shows you exactly what you missed. Starts at $9, one time.</p>
          </div>
      ...
      $('#r-review').addEventListener('click',()=>{
        if(reviewLocked){
          Paywall.open({ title:'See what you got wrong',
            body:`You got ${r.correct} of ${r.total} right. Pro shows you every question, the right answer, and why your answer was wrong. This is how your score actually goes up.` });
          return;
        }
        Review.render(r);
      });
    ...
    render(){
      ...
      const banner = pro ? `
        ...
        <div class="tier-banner">
          <span class="tier-ic">§</span>
          <div><b>${simsLeft?'Your first practice test is free':'You used your free test'}</b>
          <span>${simsLeft
            ?'No card, no signup tricks. Just take it and see your real score.'
            :'Most people need more than one try. Pro gives you unlimited tests, starting at $9.'}</span></div>
          <button class="btn primary" id="h-upgrade">See plans →</button>
        </div>`;

      $('#view-home').innerHTML=`
        <div class="hero">
          <h1>Practice the <span class="grad">CCAT</span><br />so test day feels easy.</h1>
          <p>The CCAT gives you 50 questions in 15 minutes. That's 18 seconds each. Most people run out of time before they run out of questions. The fix is simple: practice under the same pressure until it stops feeling new. Start with a full practice test — it's free, and it shows you exactly where you stand.</p>
          <div class="pills">
        ...
        <div class="section-title">Practice Modes<span class="sub">New here? Start with the full test. It shows you what to work on.</span></div>
        <div class="grid cols-3">
          <div class="card mode-card" data-go="full">
            <div class="ic">I</div><h3>Full Simulation</h3>
            <p>Don't know what to expect? This is the real test, timed the same way. 50 questions, 15 minutes, no do-overs.</p>
            <div class="go">Start the test →</div>
          </div>
          <div class="card mode-card" data-go="category">
            <div class="ic">II</div><h3>Practice by Topic</h3>
            <p>Bad at the shape puzzles, or the word questions? Pick one topic and practice just that, with the answer explained every time.</p>
            <div class="go">Choose a topic →</div>
          </div>
          <div class="card mode-card" data-go="drill">
            <div class="ic">III</div><h3>Untimed Drills</h3>
            <p>Math too slow? Practice with no clock first. Speed comes later, once the steps feel easy.</p>
            <div class="go">Start drilling →</div>
          </div>
          <div class="card mode-card" data-go="quick">
            <div class="ic">IV</div><h3>Quick 10</h3>
            <p>Only have a few minutes? Do 10 questions in 3 minutes. Good for a lunch break.</p>
            <div class="go">Quick test →</div>
          </div>
          <div class="card mode-card" data-go="study">
            <div class="ic">V</div><h3>Study Guide</h3>
            <p>Not sure what's even on this test? Read this first. 20 minutes, and you'll know what to expect.</p>
            <div class="go">Read the guide →</div>
          </div>
          <div class="card mode-card" data-go="progress">
            <div class="ic">VI</div><h3>My Progress</h3>
            <p>Want proof you're getting better? See your scores over time, and which section still needs work.</p>
            <div class="go">View progress →</div>
          </div>
        </div>

        ${pro ? '' : `
        <div class="section-title">Go Pro<span class="sub">Pay once. No monthly fee, nothing to cancel.</span></div>
        ${Paywall.pricingHTML()}`}`;
  ...
  const Pricing = {
    render(){
      ...
      $('#view-pricing').innerHTML=`
        <div class="section-title">Pricing<span class="sub">Pay once. No subscription, nothing to cancel.</span></div>
        ...
        <div class="card guide">
          <h3>My test is in two days. Can this actually help?</h3>
          <p>Yes. Tonight: read the study guide (20 minutes), then take one full practice test, then look at what you got wrong. Sleep. Take another one tomorrow. Getting used to the format is the fastest way to raise your score, especially this close to test day.</p>
          <h3>Why am I so nervous about a 15-minute test?</h3>
          <p>Because 18 seconds a question is fast, and nobody warns you about that part. It feels scary the first time. By your third practice test, it won't.</p>
          <h3>Is this a subscription?</h3>
          <p>No. You pay once. The Sprint plan lasts 7 days. Lifetime never ends. Either way, nothing renews and there's nothing to cancel.</p>
          <h3>How fast do I get access after I pay?</h3>
          <p>Right away. As soon as your payment goes through, your account unlocks. No waiting.</p>
          <h3>Do you see my card number?</h3>
          <p>No. Stripe handles your payment. We only find out if it worked.</p>
          <h3>Will I see the exact questions from my real test?</h3>
          <p>No, and don't trust anyone who says they can promise that. These questions match the same style, mix, and time pressure as the real CCAT. That's what actually helps.</p>
          <h3>What if this doesn't help me?</h3>
          <p>Email us within 14 days and we'll refund you. No questions asked.</p>
        </div>`;
      ...
      const draw=()=>{
        ...
        <div class="card">
          ...
          ${Access.isPro() ? '' : `<div class="tier-note">
            Free gives you <b>${counts.free}</b> of ${counts.total} ${CATS[cat].name.toLowerCase()} questions, ${max} at a time.
            <button class="linkish" id="c-upgrade">Unlock all of them →</button>
          </div>`}
          <div class="btn-row">
        ...
        $$('#c-count .choice').forEach(b=>b.onclick=()=>{
          if(b.dataset.lock==='1'){
            Paywall.open({ title:'That many questions is a Pro feature',
              body:`Free practice is capped at ${max} questions at a time. That's enough to learn the basics. Pro removes the cap so you can practice as much as you want.` });
            return;
          }
    ...
    drill(){
      ...
      const draw=()=>{
        ...
        $$('#d-len .choice').forEach(b=>b.onclick=()=>{
          if(b.dataset.lock==='1'){
            Paywall.open({ title:'Endless drills are a Pro feature',
              body:`Free drills stop at ${cap} questions. Pro takes the limit off, so you can keep going until the questions feel easy.` });
            return;
          }
